...
class WhollyWoodDOTADataset(CustomDataset):

    def __init__(self, *arg, balance_category=False, rhp=(1, 0, 0), version='1', **kwargs):
        ...
        self.point_proportion = rhp[2]
        self.hbox_proportion = rhp[1]
        self.point_dummy = 0.1
        self.hbox_dummy = 0
    ...
    def wood_transform(self, results):
        """The transform function."""

        results['bboxes'] = np.float32(results['bboxes'])
        max_idx_p = int(round(results['bboxes'].shape[0] * self.point_proportion))
        results['bboxes'][:max_idx_p, 2] = self.point_dummy
        results['bboxes'][:max_idx_p, 3] = self.point_dummy
        results['bboxes'][:max_idx_p, 4] = 0

        max_idx_h = max_idx_p + int(round(results['bboxes'].shape[0] * self.hbox_proportion))
        hbox = self.rotated_box_to_hbbox_np(results['bboxes'][max_idx_p:max_idx_h])
        results['bboxes'][max_idx_p:max_idx_h] = hbox
        results['bboxes'][max_idx_p:max_idx_h, 4] = self.hbox_dummy

        results['wstypes'] = np.zeros(results['bboxes'].shape[0], dtype=np.int32)
        results['wstypes'][:max_idx_p] = 2
        results['wstypes'][max_idx_p:max_idx_h] = 1
    ...
    def _read_ann_info(self,idx):
        ...
        ann = dict(
            ...
            wstypes=anno['wstypes'].astype(np.int32),
...
def test_eval():
    ...
    # results = results
    dataset = WhollyWoodDOTADataset(annotations_file='/mnt/disk/lxl/dataset/DOTA_1024/trainval_split/trainval1024.pkl',
